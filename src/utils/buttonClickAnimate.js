export function buttonClickAnimate(e) {
  function createParticle(x, y, type) {
    ...
    switch (type) {
      ...
      case 'logo':
        ...
        break
      case 'shadow':
        let color = `hsl(${Math.random() * 2}, 100%, 63%)` // Цвет hsl(${Math.random() * 50 + 200}, 70%, 50%)
        ...
        break
      case 'line':
        particle.style.background = `hsl(${Math.random() * 20 + 110}, 84%, 41%)` // Цвет линий hsl(${Math.random() * 50 + 200}, 70%, 50%)
        height = 1 // Размер
        rotation += 1000
